CREATE OR REPLACE PACKAGE packagetwofuncs AS
    PROCEDURE insert1(p_title varchar2);
    FUNCTION inject2(p_title varchar2) RETURN NUMBER;
END packagetwofuncs;
/
...
CREATE OR REPLACE PACKAGE BODY packagetwofuncs AS
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE stmt; -- No issue, not called with tainted data from 'insert1'
    end insert4;
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE 'insert into book_tbl (title, description) values (''' || p_title || ''', ''description of book inserted from inject2'')'; -- No issue, not directly callable
        OKNAME := 'TAOCP';
        ...
        EXECUTE IMMEDIATE 'insert into book_tbl (title, description) values (''' || badname || ''', ''description of book inserted from inject2'')'; -- No issue, not directly callable
        RETURN 1;
    END inject1;
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE stmt; -- No issue, not directly callable
    end insert2;
