...
begin
    ...
    for i in 1..utl_http.get_header_count(hrs) loop
        utl_http.get_header(name => hdrname, value => hdrvalue, r => hrs, n => i);
    ...
    loop
        utl_http.read_line(data => data, r => hrs);
...
begin
    hrq := UTL_HTTP.BEGIN_REQUEST(http_version => null, url => 'http://'|| host, method => 'GET');        -- CWEID 441
    UTL_HTTP.SET_AUTHENTICATION(username => username, password => password, r => hrq);       -- this is okay; variable derived from elsewhere
    UTL_HTTP.SET_AUTHENTICATION(username => 'admin', password => 'Secret123!', r => hrq);    -- CWEID 798
    UTL_HTTP.SET_AUTHENTICATION(username => 'admin', password => password, r => hrq);    -- CWEID 798
    UTL_HTTP.SET_AUTHENTICATION(username => username, password => 'Secret123!', r => hrq);    -- CWEID 798
    UTL_HTTP.SET_HEADER(hrq, 'X-Host', 'http://www.veracode.com');
    UTL_HTTP.SET_HEADER(value => 'hmm ' || username, r => hrq, name => 'X-Foo');              -- CWEID 441
    gooddata := upper('nothing to see here');
    hrs := utl_http.get_response(hrq);
    for i in 1..utl_http.get_header_count(hrs) loop
...
begin
    ...
    loop
        utl_http.read_raw(data => data, r => hrs);
...
begin
    ...
    loop
        utl_http.read_text(data => data, r => hrs);
        dbms_output.put_line(data);
    end loop;
    execute immediate substr(data, 5, 100);                 -- CWEID 89
    execute immediate substr(gooddata, 5, 100);
...
begin
    ...
    baddata0 := utl_http.request(url => 'http://www.example.com', https_host => 'foo', proxy => proxy, wallet_path => 'wallet.dat', wallet_password => password);        -- CWEID 441
    ...
    baddata1 := utl_http.request(wallet_path => 'wallet.dat', url => site, proxy => proxy);        -- CWEID 441
    ...
    execute immediate substr(baddata2, 10, 1000);
...
begin
    ...
    utl_http.set_wallet(password => 'wallet.dat', path => password);     -- CWEID 798
    ...
    badp1 := utl_http.request_pieces(max_pieces => 31337, url => site);        -- CWEID 441
    for i in 1..badp1.count LOOP
        execute immediate badp1(i);     -- CWEID 89
    END LOOP;

    badp2 := utl_http.request_pieces(max_pieces => 31337, url => 'http://www.example.com', proxy => proxy);       -- CWEID 441
